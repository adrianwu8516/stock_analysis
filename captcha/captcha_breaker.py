...
def captcha_mdfk(img_file):
    ...
    final_ans = ""
    with open('mlp_test_result2mlp.pickle', 'rb') as f:
        mlp = pickle.load(f)
        for id, (x, y, w, h) in enumerate(ary):
            ...
            digit_ary = np.array([pixel for pixel in res]).reshape(1, 2500)
            mlp_ans = mlp.predict(digit_ary)[0]
...
for file in os.listdir('img')[100:150]:
    try:
